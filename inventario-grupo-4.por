programa
{
	...
	funcao localizarCodigo(cadeia vetor[], inteiro vQuant[])
	{
		...
		faca
		{
			...
			enquanto (continuar e resposta != "s")
			{
				escreva("\nVerificar novamente? (S/N)\n--> ")
				leia(resposta)
				resposta = t.caixa_baixa(resposta)
				...
				senao se (resposta == "n") continuar = falso
			}
		 	resposta = ""
		 }
		 enquanto(continuar)
	...
	funcao menuProduto()
	{
    	...
    	funcao adicionarQuantTodos()
    	{
    		...
    		se(vQuantidadeCheio())
    		{
    			escreva(
    				...
    				"Selecione [2] para substituí-los individualmente.\n"
    			)
    		}
    	}
		...
		inteiro qntdInt = -1
    		...
    		enquanto (codigoInt < 1 ou codigoInt > T)
    		{
    			...
    			se (codigoInt < 1 ou codigoInt > T)
    			{ 
    				write("-", 37)
    				escreva("\nCódigo inválido. Valores entre 1 e ", T, ".\n")
    			}
    		}
		...
		enquanto (qntdInt < 0)
		{
			...
			write("-", 49)
			escreva("\n")
	 	   	//escreva(vetor[(codigoInt -1)], "\n")// exibir o produto selecionado 	
	 	   	imprimirProduto(codigoInt-1)
	 	   	write("-", 49)
	 	   	escreva("\n") 	 
	...
	funcao imprimirProduto(inteiro n)
	{
		se (vProdutos[n] == "")
			{
			...
			senao
			{
				se (n < 9) escreva ("  0", n + 1)
				senao escreva ("  ", n + 1)
				
				escreva("   -   ",vProdutos[n], "      ") 
				
				se (vQuantidade[n] == -1) escreva("   0\n")
				senao escreva(vQuantidade[n], "\n")
			}
	}
	...
	funcao escolherTipoExcluir()
	{
		...
		faca
		{
			cabecalho()
			escreva(
				"[1] Excluir todo o estoque\n",
				"[2] Excluir estoque por índice\n",
				"[3] Sair\n--> "
			)
	...
	{		
			...
			cadeia confirmar, tecla
			logico opcao = falso
			faca{			
			escreva("\nDeseja realmente zerar o estoque? Digite 'S' para confirmar e 'N' para sair: ")
			leia(confirmar)
			confirmar = t.caixa_baixa(confirmar)
			...
			se(confirmar == "s"){
				...
				escreva("Estoque zerado com sucesso .\n")
				opcao = verdadeiro				
		     } senao se(confirmar == "n"){
		     	opcao = verdadeiro
		     	escreva("Estoque não foi alterado")
	...
	funcao excluirCadastroIndividual()
	{
		...
		enquanto (indiceInt < 1 ou indiceInt > T)
		{
			...
			se (indiceInt < 1 ou indiceInt > T)
			{
				escreva("Código inválido. Valores entre 1 e ", T, ".\n")
			}
		}
		se (vQuantidade[indiceInt - 1] == 0) escreva("Produto já se encontra com estoque zerado.\n")
		senao vQuantidade[indiceInt - 1] = 0
			escreva("Estoque o produto zerado com sucesso .\n")
	}
 ...
 * 
 * @POSICAO-CURSOR = 310; 
 * @DOBRAMENTO-CODIGO = [33, 52, 65, 100, 134, 142, 220, 205, 257, 235, 287, 266, 308, 324, 331, 352, 370, 386, 394, 403, 411, 425, 448, 457, 464, 498, 530];
 * @PONTOS-DE-PARADA = ;
 * @SIMBOLOS-INSPECIONADOS = {vProdutos, 31, 8, 9}-{vQuantidade, 32, 9, 11};
